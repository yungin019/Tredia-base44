...
export default function CandlestickChart({ symbol, timeframe = '1D', initialData = [] }) {
  ...
  const [dataSource, setDataSource] = useState(null); // 'polygon'|'alphavantage'|'finnhub'|'unavailable'
  const canvasRef = useRef(null);
  // Cache: key -> chartData, so switching back to a seen timeframe is instant
  ...
  useEffect(() => {
    ...
    // Return cached data immediately (avoids redundant API calls on rapid switching)
    if (cacheRef.current[key]) {
      setData(cacheRef.current[key].data);
      setDataSource(cacheRef.current[key].source);
      return;
    }
    ...
    base44.functions.invoke('stockPrices', { symbol, mode: 'ohlc', timeframe })
      .then(res => {
        ...
        const src = res?.data?.source || 'unavailable';
        setDataSource(src);
        const validData = (cd && cd.length > 0) ? cd : [];
        setData(validData);
        // Cache result (including empty/unavailable so we don't re-fetch)
        cacheRef.current[key] = { data: validData, source: src };
      })
      .catch(() => {
        setDataSource('unavailable');
        cacheRef.current[key] = { data: [], source: 'unavailable' };
      })
