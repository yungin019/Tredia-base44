        ...
        if (res?.prices) {
          const liveIndices = INDEX_SYMBOLS
            ...
            .map(s => {
              ...
              return { symbol: s.symbol, name: s.name, price: data.price, change: parseFloat(change.toFixed(2)), chartData: sparkline };
            });
          if (liveIndices.length > 0) setIndices(liveIndices);
        }
      } catch (error) {
    ...
    <div className="overflow-x-auto -mx-4 px-4 scrollbar-hide mb-5">
      <div className="flex gap-3 pb-2">
        {indices.map((index, i) => {
          ...
          return (
            <motion.div key={index.symbol} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }}
              ...
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="text-[10px] font-bold text-white/40 uppercase tracking-wider mb-1">{index.name}</div>
                  <div className="font-mono font-black text-xl text-white/95">{index.price.toLocaleString(undefined, { maximumFractionDigits: 2 })}</div>
                </div>
                <div className={`flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-lg ${isUp ? 'text-chart-3 bg-chart-3/10' : 'text-destructive bg-destructive/10'}`}>
